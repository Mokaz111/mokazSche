...
import (
	...
	"mokazSche/cmd/controller/app/options"
	"mokazSche/pkg/controller/annotator"
	"net/http"
	"os"
	"time"
)
...
func NewControllerCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use: "controller",
		Long: `The  Scheduler Controller is a kubernetes controller, which is used for annotating
...
func Run(cc *controllerappconfig.CompletedConfig, stopCh <-chan struct{}) error {
	...
	healthz.InstallHandler(healthMux, healthz.NamedCheck("Moka-scheduler-controller", healthz.PingHealthz.Check))
	go func() {
