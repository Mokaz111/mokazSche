...
func (n *nodeController) processNextWorkItem() bool {
	...
	forget, err := n.sysNode(key.(string))
	if err != nil {
...
func (n *nodeController) sysNode(key string) (bool, error) {
	startTime := time.Now()
	defer func() {
...
func annotateNodeHotValue(kubeClient clientset.Interface, br *BindingRecords, node *v1.Node, schedulerPolicy policy.DynamicSchedulerPolicy) error {
	var value int

	for _, p := range schedulerPolicy.Spec.HotValue {
...
func patchNodeAnnotation(kubeClient clientset.Interface, node *v1.Node, key string, value string) error {
